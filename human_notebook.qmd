...
chart(data=biometry, genre ~ activite_physique %fill=% genre)+ 
  ...
  labs (title= "Distribution de l'activité physique en fonction du genre")
```

Les hommes ont une activité physique plutôt diversifiée mais sans valeurs extrêmes. La plupart d'entre eux ayant néanmoins un faible niveau. 
À l’inverse, il y a deux grands extrêmes chez les les femmes, traduisant une plus grande diversité dans leurs pratiques. Énormément de femmes ont une activité physique assez faible voir nulle alors que quelques unes d'entre elles s'avèrent avoir un niveaux beaucoup plus élevé. 
...
chart(data = biometry, ~ sommeil_rec_rec %fill=% age_rec_rec) +
  ...
  labs(fill = "Âge") +
  scale_fill_viridis_d() +
  labs (title = "Qualité du sommeil en fonction de l'âge",y = "Effectifs", x = "Qualité du sommeil")
```
